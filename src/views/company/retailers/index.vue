<template>
  <div class="h-full flex flex-col">
    <breadcrumbs :items="breadItems" :title="$t('LABELS.Retailers')" />
    ...
    >
      ...
      <base-filter
        ...
        :inputs="[]"
        :keyword="true"
        :btn-name="t(`BUTTONS.add`, { name: t('LABELS.Retailer') })"
        icon="fas fa-plus"
        @action="$router.push('/retailers/form')"
      />
      ...
      >
          ...
          >
            ...
            <div class="flex items-center justify-center mt-7 gap-2 flex-wrap">
              ...
              >
                ...
                {{ $t(`BUTTONS.add`, { name: $t("LABELS.Retailer") }) }}
              </router-link>
            </div>
          </div>
        ...
        <template v-slot:[`item.actions`]="{ item, index }">
          <div class="flex items-center gap-2">
            <router-link :to="`/retailers/show/${item.id}`">
              <svg-icon name="eye2" />
            </router-link>
            <router-link :to="`/retailers/form/${item.id}`">
              <svg-icon name="edit" />
            </router-link>
            <Deleter
              :url="`users/${item.id}`"
...
function cancelOrder(values, id) {
  ...
  axios
    .post(`change-user-status/${id}`, frmData)
    .then((res) => {
