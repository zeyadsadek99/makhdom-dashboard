<template>
  <div>
    ...
    <div class="space-y-6" v-else-if="retailer">
      <div class="flex flex-wrap justify-between">
        <breadcrumbs
          :items="breads"
          :title="retailer?.full_name"
          back="/retailers"
        />
      </div>

      <div
        v-if="retailer.status == 'pending'"
      ...
      <provider-details :retailer="retailer" />
      <retailer-details :item="retailer" />
      ...
      <Map
        ...
        :draggable="false"
        :location="{
          lat: retailer.company.lat,
          lng: retailer.company.lng,
        }"
      />
    </div>
    <Teleport to="body">
...
import RetailerDetails from "./show/RetailerDetails.vue";
// import Recept from "@/layouts/pos/order/Recept.vue";
  ...
  axios
    ...
    .then((res) => {
      ...
      breads.value = [
        ...
        {
          path: `/retailers/show/${route.params.id}`,
          imgIcon: "",
          name: retailer.value.full_name,
        },
      ];
      loading.value = false;
    })
...
function cancelUser(values) {
  ...
  axios
    .post(`change-user-status/${retailer.value.id}`, frmData)
    .then((res) => {
...
function acceptUser() {
  ...
  axios
    .post(`change-user-status/${retailer.value.id}`, {
      status: "accepted",
    })
    .then((res) => {
