<template>
  <div class="h-full">
    <div class="flex items-center gap-1 justify-between flex-wrap mb-7">
      <breadcrumbs
        :items="breadItems"
      ...
      <div :class="!items.length ? 'h-full' : ''" v-else>
        ...
        >
          <ShippingCard
            @remove="remove"
            @reload="fetchData"
            @edit="$router.push(`/shipping-companies/form/${item.id}`)"
            v-for="item in items"
            :key="item.id"
            :item="item"
            url="shipping_companies"
            :activate="`shipping_companies/${item.id}/toggle-active-item`"
          />
        </div>
        <div
          v-else
...
<script setup>
import ShippingCard from "@/components/cards/ShippingCard.vue";
