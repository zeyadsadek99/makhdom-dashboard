<template>
  <div class="h-full flex flex-col">
    <breadcrumbs :items="breadItems" :title="$t('LABELS.Faqs')" />
    <div
    ...
    >
      <base-filter
        name="Faqs"
        :inputs="[]"
        :btn-name="t(`BUTTONS.add`, { name: t('LABELS.faqs') })"
        icon="fas fa-plus"
        :keyword="true"
        @action="$router.push('/faqs/form')"
      />
      ...
      >
        ...
        <template v-slot:no-data>
          ...
          >
            ...
            <div class="flex items-center justify-center mt-7 gap-2 flex-wrap">
              <router-link to="/faqs/form" class="base-btn rounded-xl self-end">
                <i class="fas fa-plus"></i>
                {{ $t(`BUTTONS.add`, { name: $t("LABELS.faqs") }) }}
              </router-link>
            </div>
          </div>
        ...
        <template v-slot:[`item.is_admin_active_user`]="{ item }">
          <global-switcher
            :id="item.id"
            :url="`faqs/${item.id}/toggle-active-user`"
            v-model:modalValue="item.is_admin_active_user"
          />
        </template>-->
        <template v-slot:[`item.actions`]="{ item, index }">
          <div class="flex items-center gap-4">
            <router-link :to="`/faqs/form/${item.id}`">
