<template>
  <div class="h-full">
    <breadcrumbs :items="breadItems" :title="$t('LABELS.Themes')" />
    <div class="flex items-center gap-1 justify-end">
      <base-filter
        :inputs="inputs"
        name="themes"
        :btn-name="t(`BUTTONS.add`, { name: t('LABELS.theme') })"
        icon="fas fa-plus"
        :keyword="false"
        @action="$router.push('/themes/form')"
      >
      ...
      <div :class="!items.length ? 'h-full' : ''" v-else>
        ...
        >
          <ThemeCard
            @remove="remove"
            @reload="fetchData"
            @edit="$router.push(`/themes/form/${item.id}`)"
            v-for="item in items"
            :key="item.id"
            :item="item"
            url="story_themes"
            :activate="`story_themes/${item.id}/toggle-active-item`"
          />
        </div>
        <div
          v-else
        ...
        >
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            >
              ...
              {{ $t(`BUTTONS.add`, { name: $t("LABELS.theme") }) }}
            </router-link>
          </div>
        </div>
      </div>
