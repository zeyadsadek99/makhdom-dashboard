...
function handleSubmit(values, actions) {
  ...
  if (initialValues.image2) {
    frmData.append("card_back[media]", initialValues.image);
  }

  frmData.append("full_name", values.name);
  frmData.append("subcategory_ids[0]", values.subCategoryId);
  frmData.append("subcategory_ids[1]", values.subCategoryId2);
...
function getData() {
  axios.get(`vendors/store/${route.params.id}`).then((res) => {
    const result = res.data.data;
    
    initialValues.name = result.full_name;
    initialValues.password = result.full_name;
    initialValues.cPassword = result.full_name;
    initialValues.subCategoryId = result.full_name;
    initialValues.subCategoryId2 = result.email;
    initialValues.phoneNumber = result.phone;
    initialValues.phoneCode = result.country;
    initialValues.bankAcc = result.role.id;
