    ...
    <breadcrumbs
      ...
      :title="
        t(`BUTTONS.${route.params.id ? 'Edit' : 'new'}`, {
          name: t('LABELS.itemDiscounts'),
        })
      "
      back="/menu-setup/item-discount"
      class="mb-7"
    />
      ...
      <div class="flex-1 w-full min-w-[250px] max-w-[800px]">
        ...
        <template v-else>
          ...
          <keep-alive>
            <OrderTypes
              :data="orderTypes"
              @returned-data="setData($event, 'step5')"
              v-if="step5"
            />
          </keep-alive>
        </template>
...
function handleSubmit() {
  ...
  let url = `item_discount`;
  const frmData = new FormData();
  if (route.params.id) {
    frmData.append("_method", "PUT");
    url = `item_discount/${route.params.id}`;
  }

  const allOrderTypes = {};
  orderTypes.value.orderTypes.map((el) => (allOrderTypes[el.name] = el.status));
  const allAggOrderTypes = {};
  orderTypes.value.orderTypesAggregators.map(
  ...
  seoDetails.value.products.map((el, index) => {
  ...
  axios
    ...
    .then((res) => {
      ...
      router.push("/menu-setup/item-discount");
    })
    .catch((e) => {
      btnLoading.value = false;
...
const breads = [
  ...
  },
  {
    ...
    path: `/menu-setup/item-discount/form${
...
function getData() {
  axios.get(`item_discount/${route.params.id}`).then((res) => {
