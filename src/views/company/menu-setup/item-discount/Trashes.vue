            ...
            <h3 class="mt-4 font-semibold text-text text-center" v-else>
              {{
                $t("TITLES.No have been added yet", {
                  name: t("BUTTONS.allTrashes") + t("TITLES.itemDiscounts"),
                })
              }}
            </h3>
          </div>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          <Teleport to="body">
            <Restore
              ...
              :url="`item_discount/restore/${item.id}`"
              @remove="
                itemToShow = '';
                remove(item.id);
                showArchive = false;
...
const breads = [
  ...
  {
    path: "/menu-setup/item-discount/archived",
    imgIcon: "",
    name: t("BUTTONS.allTrashes", { name: t("TITLES.itemDiscounts") }),
  },
];
