<template>
  <div class="h-full flex flex-col">
    <breadcrumbs :items="breadItems" :title="$t('LABELS.JoiningRequests')" />
    ...
    >
      ...
      >
        ...
        <template v-slot:[`item.phone`]="{ item }">
          <small-details-card :title="item.phone_code + ' ' + item.phone" />
        </template>
...
const getStatusColor = (status) => {
  switch (status) {
    ...
    case "placed":
      return "text-primary bg-[#5A6AE81A]";
    case "refused":
