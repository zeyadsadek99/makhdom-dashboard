<template>
  <div>
    <breadcrumbs back="/links" :title="$t('LABELS.links')" :items="breads" />
    <div class="flex gap-4 flex-wrap">
      <div class="flex-1 w-full min-w-[250px]">
        <FormSkelton v-if="loading" />
        <template v-else>
          <base-card1
            :title="$t('TITLES.Details', { name: $t('LABELS.links') })"
          >
            <VeeForm
...
const breads = [
  ...
  },
  {
    ...
    path: `/links/form${route.params.id ? "/" + route.params.id : ""}`,
  },
];

function getData() {
  axios.get(`links/${route.params.id}`).then((res) => {
...
function getCategories() {
  axios.get("links-without-pagination").then((res) => {
