<template>
  <div class="h-full flex flex-col">
    <breadcrumbs :items="breadItems" :title="$t('LABELS.links')" />
    <div
    ...
    >
      <base-filter
        name="links"
        :inputs="[]"
        :btn-name="t(`BUTTONS.add`, { name: t('LABELS.links') })"
        icon="fas fa-plus"
        :keyword="true"
        @action="$router.push('/links/form')"
      />
      ...
      >
          ...
          >
            ...
            <div class="flex items-center justify-center mt-7 gap-2 flex-wrap">
              ...
              >
                ...
                {{ $t(`BUTTONS.add`, { name: $t("LABELS.links") }) }}
              </router-link>
            </div>
          </div>
        ...
        <template v-slot:[`item.type`]="{ item }">
          <div class="flex gap-2 items-center flex-wrap">
            <small-details-card :title="`${item.type}`" />
          </div>
        </template>
        <template v-slot:[`item.action`]="{ item, index }">
          <div class="flex items-center gap-4">
            <router-link :to="`/links/form/${item.id}`">
...
const headers = [
  {
    title: t("LABELS.Name", { name: t("LABELS.link") }),
    align: "start",
    sortable: false,
    key: "link",
  },
